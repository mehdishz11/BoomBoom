package psb.com.kidpaint.utils.customView.colorPicker;
...
/**
 * Created by Mehdi on 2/7/2018 AD.
 */
...
public class ColorPicker extends FrameLayout {
    ...
    private void init() {
        ...
        swatchStrokePaint.setStrokeWidth(Value.dp(5));
        ...
        undoButton = new ImageView(getContext());
        undoButton.setScaleType(ImageView.ScaleType.CENTER);
        undoButton.setImageResource(R.drawable.icon_undo);
        ...
        location = getContext().getSharedPreferences("paint", Activity.MODE_PRIVATE).getFloat("last_color_location", 1.0f);
        setLocation(location);
    }

    private static int getSize(float size) {
        return (int) (size < 0 ? size : Value.dp(size));
    }
    ...
    public void addUndoButton() {
        addView(undoButton, new LayoutParams(getSize(60), getSize(52)));
    }

    public void addBrushButtons() {
        addView(settingsButton, new LayoutParams(getSize(60), getSize(52)));
    }
    ...
    private int interpolateColors(int leftColor, int rightColor, float factor) {
        ...
        Log.d(App.TAG, "interpolateColors: r:"+r+"g:"+g+"b:"+b);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_POINTER_UP) {
            ...
            if (y < -Value.dp(10)) {
                y=getY();
                changingWeight = true;
                float weightLocation = (-y - Value.dp(10)) / Value.dp(190);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        gradientPaint.setShader(new LinearGradient(Value.dp(56), 0, width - Value.dp(56), 0, COLORS, LOCATIONS, Shader.TileMode.REPEAT));
        int y = height - Value.dp(32);
        rectF.set(Value.dp(56), y, width - Value.dp(56), y + Value.dp(12));

        settingsButton.layout(width - settingsButton.getMeasuredWidth(), height - Value.dp(52), width, height);
        undoButton.layout(0, height - Value.dp(52), settingsButton.getMeasuredWidth(), height);
    }

    @Override
    protected void onDraw(Canvas canvas) {
        canvas.drawRoundRect(rectF, Value.dp(6), Value.dp(6), gradientPaint);

        changingWeight=false;
        weight=0.5f;

        int cx = (int) (rectF.left + rectF.width() * location);
        int cy = (int) (rectF.centerY() + draggingFactor * -Value.dp(50) - (changingWeight ? weight * Value.dp(190) : 0.0f));


        int side = (int) (Value.dp(24) * (0.5f * (1 + draggingFactor)));
        shadowDrawable.setBounds(cx - side, cy - side, cx + side, cy + side);
        shadowDrawable.draw(canvas);

        float swatchRadius = (int) Math.floor(Value.dp(4) + (Value.dp(19) - Value.dp(4)) * weight) * (1 + draggingFactor) / 2;

        canvas.drawCircle(cx, cy, Value.dp(22) / 2 * (draggingFactor + 1), backgroundPaint);
        canvas.drawCircle(cx, cy, swatchRadius, swatchPaint);
        canvas.drawCircle(cx, cy, swatchRadius - Value.dp(0.5f), swatchStrokePaint);
    }
